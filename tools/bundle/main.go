...
func main() {
	...
	for _, art := range m.Artifacts {
		...
		if err := writeArtifact(outFile, content); err != nil {
...
func preamble(m *Manifest) string {
	var b strings.Builder
	b.WriteString("// Code generated by tools/bundle; DO NOT EDIT.\n")
...
func writeArtifact(outFile *pathlib.Path, content string) error {
	if _, err := pathlib.MkDirWithOptions(outFile.Parent(), pathlib.DirOptions{ExistOk: true, CreateAll: true}); err != nil {
		return err
	}
	if _, err := pathlib.CreateFileWithOptions(outFile, pathlib.FileOptions{ExistOk: true}); err != nil {
		return err
	}
