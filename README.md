![Repository Banner](docs/assets/repo-banner.png)
...
_A simple library for handling filesystem paths._

<br>

The API is inspired by Python's [pathlib](https://docs.python.org/3/library/pathlib.html) and abstracts the standard library by providing a simple immutable struct that acts as a source of truth for file paths. go-pathlib provides single-file bundle library files that users can use instead of adding go-pathlib to their go.mod. These bundles can be downloaded from the release tab, or by running the bundler at `tools/bundle/main.go`.

The library is developed and tested on Unix- and Windows-based operating systems.
...
# Artwork and Attributions

The artwork visible in the banner is generated using Gemini Banana. The composition and text are added manually. Used font: _Afacad_ by Kristian Möller and Dicotype, available on [Github](https://github.com/Dicotype/Afacad) and [Google Fonts](https://fonts.google.com/specimen/Afacad), licensed under [SIL OPEN FONT LICENSE. Version 1.1](https://openfontlicense.org/open-font-license-official-text/). Background color: `#A3D9E3`.
